...
// everything: they mean that the embedded plugin.
func TestPluginStepCopiesIntoTheHarnessSkillsDirectory(t *testing.T) {
	fetcher := newFakePluginFetcher()

	report, err := NewInitialize(settled(""), toolsInstalled(), fetcher).Run(t.Context(), pluginRequest(), nil)
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
// A claude-code harness that cannot be read runs no plugin step: the report commands stay empty.
// Its companion tests for enforcement about manifest write aside — done behavior.
func TestPluginStepStopsTheRunWhenTheCopyFails(t *testing.T) {
	fetcher := newFakePluginFetcher()
	fetcher.err = errors.New("disk full")

	_, err := NewInitialize(settled(""), toolsInstalled(), fetcher).Run(t.Context(), pluginRequest(), nil)
	if err == nil ||
