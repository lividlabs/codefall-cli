...
`bd dolt push` move beads between machines; git does not.

- Track every task in `bd`, never in TodoWrite, TaskCreate, or a markdown TODO list.
- `bd dolt pull` before reading the graph. `codefall-refresh` does it when it runs.
- `bd dolt push` after every write, a claim included, and before the work that follows it.
...
- `.beads/interactions.jsonl` is bd's append-only log and lands in the next commit after a bead
  write. Never edit it, resolve a conflict in it by hand, or review it.
