# codefall-conceptualize — where it came from
...
pretending to have resolved something is a defect, and a concept with four open questions has done
its job.
...
nothing down about the problem any of it solves — and that person needs a concept more than most,
because the reasoning exists only in their head.

## The floor is advice, not a gate

One bug, one screen, one endpoint, one field is usually smaller than a concept is worth. But the
user knows things about the work that the skill does not, and a concept nobody needed costs a file.
So the skill says what it thinks and writes the concept if that is what they want.

## Siblings, not a parent

Each problem that stands on its own is its own concept, and what connects siblings is the source
they came from: each one's `Related` line points at the same saved document or mockup directory.
The `specs` field on `Related` is a list for the same reason — one concept can produce several
specifications, and the concept is what groups them, which is why `codefall-specify` writes sibling
...
the concept. That is the one transition another verb makes, and the carve-out the repo's rules allow
for recording a fact. A concept still at `Ready` means `codefall-implement` has not run against it.
Two optional header fields, `Revised by` and `Replaced by`, exist because a concept can be under
active work and partly revised at the same time, and a single status line cannot say both.

## `docs/concepts/AGENTS.md` stands in for an ignore file
...
concept that prompted it. The import procedure is shared with `codefall-specify` and
`codefall-mock-up` so the three never drift.

## The source is always saved

The source records what the user actually brought; the concept records what was made of it, and
...
Someone who has a vision document will say so. Chasing material the user did not mention wastes the
